/**
 * Rayman2 Mod Loader
 * Parse Cheatcode file (codes.dat and patches.dat) generated by the Manager using "codes.lst".
 */
...
void InitCodeFile(const std::wstring path, const char* debug) {
	...
	if (codes_str.is_open()) {
		...
		if (!memcmp(buf, codemagic, sizeof(codemagic))) { // valid code file
			...
			// codecode is either the number of codes (>= 0) loaded or the error (negative.)
			if (codecount >= 0) {
